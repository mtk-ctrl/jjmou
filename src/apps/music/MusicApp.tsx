...
const STORAGE_KEY = (user: string) => `musicprog_stars_${user}_v1`;
const UNLOCKED_KEY = (user: string) => `musicprog_unlocked_${user}_v1`;
...
function fireConfetti() {
  ...
  const colors = ['#a29bfe','#6c5ce7','#ffd700','#00ff88','#ff6cae','#0ff'];
  const frame = () => {
...
const LEVEL_GROUP_TITLES = ['ドだけ','ドとレ','ドレミ','ドレミファ','5音・ループ',
  'ソラシ','童謡(かんたん)','童謡(ふつう)','複雑なループ','マスター'];

export default function MusicApp({ onHome, currentUser }: { onHome: () => void; currentUser: string }) {
  ...
  const handleReset = () => { clearTimers(); setGameState('idle'); setActiveIndex(null); setPlayingNote(null); };
  const handleNext = () => {
    if (levelIndex < MUSIC_LEVELS.length - 1) {
      setLevelIndex(prev => prev + 1); setCommands([]); setGameState('idle'); setActiveIndex(null);
    }
  };
  const handleRetry = () => { clearTimers(); setCommands([]); setGameState('idle'); setActiveIndex(null); };

  const totalStars = Object.values(stars).reduce((a, b) => a + b, 0);

  const levelGroups = Array.from({ length: 10 }, (_, l) => ({
    levelNum: l + 1,
    levels: MUSIC_LEVELS.filter(lv => lv.levelNum === l + 1),
    firstIdx: MUSIC_LEVELS.findIndex(lv => lv.levelNum === l + 1),
  }));
  ...
  return (
    <div style={{
      ...
      <motion.div key={levelIndex} initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}}
        ...
        <div style={{fontSize:11,color:'#a29bfe',marginTop:2}}>{'⭐'.repeat(stars[levelIndex]??0)}{'☆'.repeat(3-(stars[levelIndex]??0))}</div>
      </motion.div>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:4}}>
          {level.target.map((note,i)=>(
            <motion.div key={i}
              ...
              style={{background:activeIndex===i&&gameState==='running'?NOTE_COLOR[note]:'rgba(255,255,255,0.12)',border:`2px solid ${NOTE_COLOR[note]}`,borderRadius:8,padding:'4px 8px',fontSize:12,fontWeight:700,color:activeIndex===i&&gameState==='running'?'#fff':NOTE_COLOR[note],transition:'all 0.2s'}}>
              {note}
            </motion.div>
          ))}
        </div>
          ...
          {commands.map(cmd=>{
            ...
            return (
              <div key={cmd.id} style={{border:'2px solid #a29bfe',borderRadius:10,padding:'4px 8px',display:'flex',flexWrap:'wrap',alignItems:'center',gap:4}}>
                ...
                <motion.button whileTap={{scale:0.85}} onClick={()=>{const notes=level.availableNotes;if(notes.length>0)handleAddNoteToLoop(notes[0],cmd.id);}}
      ...
      <div style={{width:'90%',maxWidth:440,margin:'4px 0'}}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
          {level.availableNotes.map(note=>(
            <motion.button key={note} whileTap={{scale:0.88}} onClick={()=>handleAddNote(note)}
              animate={playingNote===note?{scale:[1,1.15,1]}:{}}
              style={{background:NOTE_COLOR[note],border:'none',borderRadius:10,padding:'8px 14px',color:'#fff',fontFamily:'inherit',fontWeight:800,fontSize:14,cursor:'pointer',boxShadow:`0 3px 10px ${NOTE_COLOR[note]}66`}}>
              {note}
            </motion.button>
          ))}
      ...
      {/* Level Select - grouped by levelNum */}
      <AnimatePresence>
        {showLevelSelect&&(
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{scale:0.9}} animate={{scale:1}} onClick={e=>e.stopPropagation()}
              style={{background:'#302b63',borderRadius:20,padding:'16px',maxWidth:380,width:'90%',maxHeight:'80vh',overflowY:'auto'}}>
              ...
              {levelGroups.map(group=>(
                <div key={group.levelNum} style={{marginBottom:10}}>
                  <div style={{fontSize:12,fontWeight:800,color:'#a29bfe',marginBottom:4}}>レベル{group.levelNum}: {LEVEL_GROUP_TITLES[group.levelNum-1]}</div>
                    ...
                    {group.levels.map((lv,qi)=>{
                      ...
                      return (
                        <motion.button key={lv.id} whileTap={unlocked?{scale:0.9}:{}} disabled={!unlocked}
                          ...
                          style={{background:i===levelIndex?'linear-gradient(135deg,#a29bfe,#6c5ce7)':unlocked?'rgba(255,255,255,0.1)':'rgba(255,255,255,0.04)',border:'1.5px solid rgba(255,255,255,0.15)',borderRadius:10,padding:'6px 8px',cursor:unlocked?'pointer':'default',fontFamily:'inherit',color:unlocked?'#fff':'#666',minWidth:44}}>
