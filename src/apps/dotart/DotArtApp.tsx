...
const STORAGE_KEY = (user: string) => `dotart_stars_${user}_v1`;
const UNLOCKED_KEY = (user: string) => `dotart_unlocked_${user}_v1`;
...
function fireConfetti() {
  ...
  const colors = ['#ffd700','#ff6b35','#00ff88','#ff6cae','#4ecdc4','#a29bfe'];
  const frame = () => {
...
function simulate(cmds: DotCmd[], level: DotLevel): SimStep[] {
  ...
  function runCmds(cs: DotCmd[]) {
    for (const c of cs) {
      if (c.type==='color') { color=c.color; }
      else if (c.type==='move') {
        const [dr,dc]=DIR_DELTA[c.dir];
        const nr=row+dr,nc=col+dc;
        if(nr>=0&&nr<level.gridSize&&nc>=0&&nc<level.gridSize) {
          row=nr;col=nc;grid[row][col]=color;
          steps.push({grid:grid.map(r=>[...r]),row,col,color});
        }
      } else { for(let i=0;i<c.count;i++) runCmds(c.children); }
    }
  }
  runCmds(cmds);
  return steps;
...
function countCmds(cmds: DotCmd[]): number {
  let n=0;
  for(const c of cmds){n++;if(c.type==='loop')n+=countCmds(c.children);}
  return n;
}
...
const LEVEL_GROUP_TITLES = ['よこせん','たてせん','かどのかたち','しかく','ループしましま',
  'たいかくせん','2色パターン','にじいろ','十字・矢印','マスター'];

export default function DotArtApp({ onHome, currentUser }: { onHome: () => void; currentUser: string }) {
  ...
  const resetGrid = useCallback((lv: DotLevel) => {
    const g:(Color|null)[][]=Array.from({length:lv.gridSize},()=>Array(lv.gridSize).fill(null));
    g[lv.startRow][lv.startCol]=lv.availableColors[0]??'red';
    setDisplayGrid(g); setCursorPos([lv.startRow,lv.startCol]);
  }, []);
  ...
  const handleRun = useCallback(() => {
    ...
    steps.forEach((step,i)=>{
      const t=setTimeout(()=>{
        setDisplayGrid(step.grid.map(r=>[...r])); setCursorPos([step.row,step.col]);
        if(i===steps.length-1){
          const t2=setTimeout(()=>{
            ...
            if(match){
              ...
              const newStars={...loadStars(currentUser),[levelIndex]:Math.max(loadStars(currentUser)[levelIndex]??0,starCount)};
              setStars(newStars); saveStars(currentUser,newStars);
              const newUnlocked=Math.max(unlockedCount,levelIndex+2);
              setUnlockedCount(newUnlocked); saveUnlocked(currentUser,newUnlocked);
  ...
  },[commands,gameState,level,levelIndex,cmdCount,currentUser,unlockedCount]);

  const addCmd=(cmd:DotCmd)=>{
    if(gameState==='running') return;
    if(addingToLoop){
      setCommands(prev=>prev.map(c=>c.id===addingToLoop&&c.type==='loop'?{...c,children:[...c.children,cmd]}:c));
    } else { setCommands(prev=>[...prev,cmd]); }
  };

  const handleRemove=(id:string)=>{
    if(gameState==='running') return;
    const remove=(cs:DotCmd[]):DotCmd[]=>cs.filter(c=>c.id!==id).map(c=>c.type==='loop'?{...c,children:remove(c.children)}:c);
    setCommands(remove);
    if(addingToLoop===id) setAddingToLoop(null);
  };

  const handleReset=()=>{clearTimers();setGameState('idle');resetGrid(level);};
  const handleRetry=()=>{clearTimers();setCommands([]);setGameState('idle');resetGrid(level);setAddingToLoop(null);};
  const handleNext=()=>{
    if(levelIndex<DOT_LEVELS.length-1){
      const next=levelIndex+1;setLevelIndex(next);setCommands([]);setGameState('idle');setAddingToLoop(null);resetGrid(DOT_LEVELS[next]);
    }
  };
  const selectLevel=(i:number)=>{
    setLevelIndex(i);setCommands([]);setGameState('idle');setAddingToLoop(null);resetGrid(DOT_LEVELS[i]);setShowLevelSelect(false);
  };
  ...
  const renderCmd=(cmd:DotCmd):React.ReactNode=>{
    if(cmd.type==='move') return(
      <motion.div key={cmd.id} initial={{scale:0}} animate={{scale:1}} whileTap={{scale:0.85}} onClick={()=>handleRemove(cmd.id)}
    ...
    if(cmd.type==='loop') return(
      <div key={cmd.id} style={{border:addingToLoop===cmd.id?'2px solid #ffd700':'2px solid #a29bfe',borderRadius:10,padding:'4px 8px',display:'flex',flexWrap:'wrap',alignItems:'center',gap:4}}>
        ...
        {cmd.children.map(child=>renderCmd(child))}
  ...
  const levelGroups = Array.from({length:10},(_,l)=>({
    levelNum:l+1,
    levels:DOT_LEVELS.filter(lv=>lv.levelNum===l+1),
    firstIdx:DOT_LEVELS.findIndex(lv=>lv.levelNum===l+1),
  }));

  return (
    <div style={{minHeight:'100dvh',background:'linear-gradient(180deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%)',display:'flex',flexDirection:'column',alignItems:'center',fontFamily:"'M PLUS Rounded 1c',sans-serif",color:'#fff',paddingBottom:12}}>
      {['🎨','✏️','🖌️','🎭','⭐'].map((e,i)=>(
        <motion.div key={i} animate={{y:[-8,8,-8],opacity:[0.3,0.7,0.3]}} transition={{repeat:Infinity,duration:3+i,delay:i*0.5}}
      ...
      <div style={{width:'100%',maxWidth:480,padding:'8px 12px 4px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <div style={{textAlign:'center'}}>
          ...
          <div style={{fontSize:10,opacity:0.6}}>Lv.{level.levelNum} Q{level.questionNum}/5　⭐{totalStars}</div>
        </div>
        ...
        <div style={{textAlign:'center'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:`repeat(${level.gridSize},${cellSize}px)`,gap:1}}>
            {displayGrid.map((row,r)=>row.map((cell,c)=>{
              ...
              return (
                <motion.div key={`${r}-${c}`} animate={isCursor&&gameState==='running'?{scale:[1,1.2,1]}:{}}
      ...
      <div style={{width:'90%',maxWidth:440,margin:'4px 0',minHeight:52,background:'rgba(255,255,255,0.05)',borderRadius:12,padding:8}}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:4}}>{commands.map(cmd=>renderCmd(cmd))}</div>
      </div>
        ...
        {showLoopModal&&(
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{scale:0.8}} animate={{scale:1}} onClick={e=>e.stopPropagation()}
              ...
              <motion.button whileTap={{scale:0.93}} onClick={()=>{addCmd({id:genId(),type:'loop',count:loopCount,children:[]});setShowLoopModal(false);}}
        ...
        {showLevelSelect&&(
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{scale:0.9}} animate={{scale:1}} onClick={e=>e.stopPropagation()}
              style={{background:'#16213e',borderRadius:20,padding:'16px',maxWidth:380,width:'90%',maxHeight:'80vh',overflowY:'auto'}}>
              ...
              {levelGroups.map(group=>(
                <div key={group.levelNum} style={{marginBottom:10}}>
                  <div style={{fontSize:12,fontWeight:800,color:'#ffd700',marginBottom:4}}>レベル{group.levelNum}: {LEVEL_GROUP_TITLES[group.levelNum-1]}</div>
                  <div style={{display:'flex',gap:4}}>
                    {group.levels.map((lv,qi)=>{
                      const i=group.firstIdx+qi;
                      const unlocked=i<unlockedCount;
                      const s=stars[i]??0;
                      return (
                        <motion.button key={lv.id} whileTap={unlocked?{scale:0.9}:{}} disabled={!unlocked}
                          onClick={()=>unlocked&&selectLevel(i)}
                          style={{background:i===levelIndex?'linear-gradient(135deg,#ffd700,#ff6b35)':unlocked?'rgba(255,255,255,0.1)':'rgba(255,255,255,0.04)',border:'1.5px solid rgba(255,255,255,0.15)',borderRadius:10,padding:'6px 8px',cursor:unlocked?'pointer':'default',fontFamily:'inherit',color:unlocked?'#fff':'#666',minWidth:44}}>
                          <div style={{fontSize:12,fontWeight:800}}>Q{lv.questionNum}</div>
                          <div style={{fontSize:8}}>{s>0?'⭐'.repeat(s):unlocked?'☆☆☆':'🔒'}</div>
                        </motion.button>
                      );
                    })}
                  </div>
